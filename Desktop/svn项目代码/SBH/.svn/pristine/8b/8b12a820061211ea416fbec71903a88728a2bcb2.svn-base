...
- (NSMutableArray *)reloadCalendarView:(NSDate *)date  selectDate:(NSDate *)selectdate withType:(DayTipsType)type
{
    customType = type;
    //如果为空就从当天的日期开始
    if(date == nil){
    ...
    today = date;//起始日期
    if(customType == DayTipsTypeTrainStart)
    {
        before = [date dayInTheFollowingDay:60];
    }
    else
    {
        before = [date dayInTheFollowingMonth:6];
    }
...
- (void)changStyle:(CalendarDayModel *)calendarDay
{
    ...
    NSDateComponents *todayComponets = [todate YMDComponents];
    ...
    if(calendarSelect.year == calendarDay.year &
       ...
       calendarSelect.day == calendarDay.day){
        calendarDay.modelType = customType;
    ...
    //这里来判断节日
        //今天
    if (todayComponets.year == calendarDay.year &&
        todayComponets.month == calendarDay.month &&
        todayComponets.day == calendarDay.day) {
        calendarDay.specialDay = @"今天";
        
        //明天
    }else if(todayComponets.year == calendarDay.year &&
             todayComponets.month == calendarDay.month &&
             todayComponets.day - calendarDay.day == -1){
        calendarDay.specialDay = @"明天";
        
        //后天
    }else if(todayComponets.year == calendarDay.year &&
             todayComponets.month == calendarDay.month &&
             todayComponets.day - calendarDay.day == -2){
        calendarDay.specialDay = @"后天";
        //1.1元旦
        ...
        //4.5清明节
#pragma mark - 每年清明节都要改
    }else if (calendarDay.month == 4)
    {
        int qingming = (calendarDay.year-2000)* 0.2422 + 4.81 - (calendarDay.year-2000)/4.0;
        if(qingming == calendarDay.day)
        {
            calendarDay.holiday = @"清明节";
        }
        //5.1劳动节
